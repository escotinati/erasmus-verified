...
La idea es que TODAS las cards de la web sean miembros de la misma familia (`Card.jsx`), distinguidas por qué slots, qué `layout` y qué `cta.kind` llevan. **Fase 1 (hecha)**: partner-card + event-card del home. **Fase 2 (hecha)**: `city-partners-card` de `ciudad.html` (`CityPartnerList.jsx` ahora solo traduce partner → props de `Card` con `layout="compact"`). Pendientes, cada una en su propia rama: `collab-card` y `service-card` (HTML estático en alojamiento/viajes/servicios), `city-card` (`ciudades.js`/`ciudades-todas.html`).

### Qué hace cada archivo

| Archivo                             | Qué hace                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/react/Card.jsx`                | La card única de la web. Slots **opcionales**: `imageUrl`/`imageAlt`/`monogram`/`badge`, `title`/`meta` (+`metaIcon`)/`date`/`price`/`description` y `ctas: [{ kind, label, href?, onClick? }]` (o `cta` suelto). Un slot sin valor no se renderiza — cada tipo de card es una combinación de slots, no un componente aparte. Prop `layout`: `'stacked'` (defecto, imagen 4:3 arriba — home) o `'compact'` (avatar 44px junto al título, con borde — partners de ciudad; `accent` = color CSS que tiñe badge, monograma y hover). `CTA_KINDS` es el único sitio donde se define cada tipo de botón: `info` (enlace de texto, se revela en hover con puntero fino), `tickets` (botón relleno siempre visible), `directions` (secundario, enlace a Google Maps) y `details` (primario; sin `href` pero con `onClick` se renderiza como `<button>`); un tipo nuevo = una entrada ahí + su modificador `.card-cta--<kind>` en `card.css`. Sanea `imageUrl`/`cta.href` con `sanitizeUrl()`. Estilos en `src/css/card.css`; lo que depende del contenedor (carrusel/grid de eventos, `.card--featured`) sigue en `home.css` bajo `.events-scroll .card`. |
