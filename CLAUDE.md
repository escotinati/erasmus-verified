...
- `profiles` — perfil de cada usuario final registrado (no admin): `id` (mismo `uuid` que `auth.users.id`), `city_id`, `university`, `interests` (array de texto: subconjunto de `nightlife`/`housing`/`travel`), `membership_tier` (`free`/`verified`/`plus` — pensado para una futura tarjeta Erasmus unificada, no se fija todavía desde ningún formulario, siempre nace en `free`). Se rellena sola vía trigger, nunca por `INSERT` explícito desde el frontend — ver [Autenticación de usuarios](#autenticación-de-usuarios-loginhtml--registrohtml).
...
- **`registro.html`** (`src/js/registro.js`): email + contraseña (mínimo 6 caracteres) + ciudad (obligatoria) + universidad de origen (texto libre, opcional) + intereses (checkboxes `nightlife`/`housing`/`travel`, opcional). La ciudad **no es un `<select>`** — es un autocompletado de texto libre (mismo patrón que `#citySearch` en `index.js`, reutilizando `.search-dropdown`/`.sdi-*` de `components.css`): el texto visible (`#reg-city`) es solo lo que teclea el usuario, el id real que se envía vive en un input oculto (`#reg-city-id`). Llama a `signUp({ email, password, cityId, university, interests })`.
- **`src/js/services/authService.js`**: capa fina sobre `supabase.auth` — `signUp()`, `signIn()`, `signOut()`, `getSession()`. `signUp()` manda `city_id`/`university`/`interests` dentro de `options.data` (metadata del usuario de Auth) — **no** hace ningún `INSERT` a `public.profiles` directamente: eso lo hace un trigger de Postgres (`handle_new_user`, ya aplicado en Supabase) que lee `raw_user_meta_data` al crearse la fila en `auth.users` y crea la fila correspondiente en `profiles` con esas mismas claves. Si se añade un campo nuevo al formulario de registro que deba llegar a `profiles`, tiene que viajar dentro de `options.data` con el nombre de clave exacto que espera el trigger.
