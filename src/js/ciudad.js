...
(async function () {
    ...
    // El CTA de WhatsApp va PRIMERO, justo debajo del <h1> — a petición
    // explícita de Álvaro: antes vivía dentro de #actionsWrap pero
    // CONCATENADO DESPUÉS del bloque de partners, así que aunque el
    ...
    let quickActions = city.whatsapp_url
        ...
        : buildGroupPending();
    quickActions += buildMapLink(city.id);

    document.getElementById('actionsWrap').innerHTML =
        `<div class="city-quick-actions">${quickActions}</div>` + buildPartnersBlock(city);
...
// El grupo de una ciudad es SIEMPRE WhatsApp (cities.whatsapp_url, no
// hay campo de Telegram — ver CLAUDE.md) — antes esta función aceptaba
...
// Compacta a propósito, coherente con buildWhatsappCta()/buildMapLink()
// en la misma fila — antes era una caja centrada grande con emoji +
// título + frase con el nombre de la ciudad; esa frase ("Todavía no
// tenemos grupos para X. Vuelve pronto.") no cabía en una pill sin
// desentonar, así que se queda solo con el título.
function buildGroupPending() {
    return `
    <span class="city-group-pending">
      <span class="material-symbols-outlined" aria-hidden="true">hourglass_empty</span>
      ${I18n.t('city.groups_coming_soon_title')}
    </span>`;
}
...
// feature/city-no-map. Solo el contenedor donde mountCityPartners()
// (cityPartners.js) monta la lista — el enlace a mapa.html vive ahora
// en buildMapLink(), dentro de .city-quick-actions junto al CTA de
// WhatsApp (ver el IIFE de arriba).
