...
if (!pais || !ciudad || !country) {
  ...
  // ── Botón del mapa: SIEMPRE visible, independiente de si hay
  // grupos WA/TG/generic. Va primero en .actions. ──
  let btns = buildMapBtn(pais, ciudad);
...
/**
 * Botón "Ver mapa de {ciudad}" — enlaza a mapa.html?pais=...&ciudad=...
 * Usa la clase .btn-generic (mismo estilo que el botón "Unirse a la
 * comunidad") más un icono de pin de mapa.
 */
function buildMapBtn(pais, ciudad) {
  const url = `mapa.html?pais=${encodeURIComponent(pais)}&ciudad=${encodeURIComponent(ciudad)}`;
  return `
    <a href="${url}" class="action-btn btn-generic">
      ${iconMap()}
      <div class="btn-label">
        <strong>Ver mapa de ${ciudad}</strong>
        <span>Bares, discotecas y puntos de interés</span>
      </div>
      <svg class="btn-chevron" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
        <polyline points="9 18 15 12 9 6"/>
      </svg>
